#!/bin/bash
# ============================================================
#  Crea l'icona "Gestionale OMPT" sulla Scrivania (macOS).
#
#  Non si lancia da solo: lo richiamano
#    - "Crea icona sul desktop.command", per chi lo cerca;
#    - "Avvia gestionale.command", che lo propone al primo avvio.
#
#  Tenerlo qui dentro evita di ripetere due volte lo stesso codice
#  e, soprattutto, permette di creare l'icona partendo da un file
#  che l'utente ha gia' sbloccato: macOS mette in quarantena ogni
#  file scaricato, quindi far sbloccare un secondo file sarebbe un
#  ostacolo in piu' per nessun motivo.
#
#  Uso:  crea_icona_mac "/percorso/della/cartella/gestionale"
#  Restituisce 0 se l'icona e' stata creata.
#
#  Il corpo non e' indentato di proposito: i delimitatori degli
#  here-document devono stare a inizio riga.
# ============================================================
...
if [ ! -d "$SCRIVANIA" ]; then
  echo "  Non trovo la cartella Scrivania: non posso creare l'icona."
  return 1
fi

rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources" || {
  echo "  Non riesco a scrivere sulla Scrivania: non posso creare l'icona."
  return 1
}
